...
func testUtils(t *testing.T, when spec.G, it spec.S) {
	...
	when(".ResolveRunImage", func() {
		when("when there are no mirrors", func() {
		...
		when("when there are mirrors", func() {
		...
		when("when there is no run image defined", func() {
			it("should fail", func() {
				md := platform.StackMetadata{}
				dstImage := "someregistry/whatever"
				_, err := lifecycle.ResolveRunImage(md, dstImage)
				h.AssertError(t, err, "CNB_RUN_IMAGE is required when there is no stack metadata available")
			})
		})
	})
}
